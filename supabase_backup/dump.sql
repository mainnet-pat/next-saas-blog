...
CREATE FUNCTION public.create_user_on_signup() RETURNS trigger
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$BEGIN
    RAISE NOTICE '%', new.id;
    ...
    VALUES (
      NEW.id,
      CASE WHEN new.raw_user_meta_data ->>'email' IS NULL THEN new.email ELSE new.raw_user_meta_data ->>'email' END,
      CASE WHEN new.raw_user_meta_data ->>'user_name' IS NULL THEN new.email ELSE new.raw_user_meta_data ->>'user_name' END,
      CASE WHEN new.raw_user_meta_data ->>'avatar_url' IS NULL THEN 'https://rogerweb.vercel.app/avatar.png' ELSE new.raw_user_meta_data ->>'avatar_url' END
    );
...
CREATE FUNCTION public.is_admin_or_owner(user_id uuid) RETURNS boolean
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$begin
  return user_id = auth.uid() OR (select role from users where id = auth.uid()) = 'admin';
end;$$;


ALTER FUNCTION public.is_admin_or_owner(user_id uuid) OWNER TO postgres;
...
ALTER FUNCTION public.is_owner(user_id uuid) OWNER TO postgres;
...
CREATE TABLE public.campaign (
    ...
    target_usd numeric DEFAULT '0'::numeric NOT NULL
);
...
--
-- Name: campaign_content; Type: TABLE; Schema: public; Owner: postgres
--
...
ALTER TABLE public.campaign_content OWNER TO postgres;
...
CREATE TABLE public.users (
    ...
    image_url text NOT NULL,
    role text DEFAULT '''user''::text'::text NOT NULL,
...
--
-- Name: campaign blog_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--
...
--
-- Name: campaign_content campaign_content_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.campaign_content
    ADD CONSTRAINT campaign_content_pkey PRIMARY KEY (campaign_id);
...
--
-- Name: users create_user_on_signup; Type: TRIGGER; Schema: auth; Owner: supabase_auth_admin
--

CREATE TRIGGER create_user_on_signup AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION public.create_user_on_signup();
...
--
-- Name: campaign_content campaign_content_campaign_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.campaign_content
    ADD CONSTRAINT campaign_content_campaign_id_fkey FOREIGN KEY (campaign_id) REFERENCES public.campaign(id) ON UPDATE CASCADE ON DELETE CASCADE;


--
-- Name: campaign_content campaign_content_user_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.campaign_content
    ADD CONSTRAINT campaign_content_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id) ON UPDATE CASCADE ON DELETE CASCADE;


--
-- Name: campaign campaign_user_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.campaign
    ADD CONSTRAINT campaign_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
--
-- Name: campaign Enable delete for admin or owner if unpublished; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Enable delete for admin or owner if unpublished" ON public.campaign FOR DELETE USING ((public.is_admin() OR (public.is_owner(user_id) AND (is_published = false))));


--
-- Name: campaign_content Enable delete for admin or owner if unpublished; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Enable delete for admin or owner if unpublished" ON public.campaign_content FOR DELETE USING ((public.is_admin() OR (public.is_owner(user_id) AND (( SELECT campaign.is_published
   FROM public.campaign
  WHERE (campaign_content.campaign_id = campaign.id)) = false))));


--
-- Name: campaign Enable insert for owners only; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Enable insert for owners only" ON public.campaign FOR INSERT TO authenticated WITH CHECK (public.is_owner(user_id));


--
-- Name: campaign_content Enable insert for owners only; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Enable insert for owners only" ON public.campaign_content FOR INSERT TO authenticated WITH CHECK (public.is_owner(user_id));


--
-- Name: users Enable read access for admins/owners; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Enable read access for admins/owners" ON public.users FOR SELECT USING (public.is_admin_or_owner(id));


--
-- Name: campaign Enable read access for all users if published or owner/admin; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Enable read access for all users if published or owner/admin" ON public.campaign FOR SELECT USING (true);


--
-- Name: campaign_content Enable read access for all users if published or owner/admin; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Enable read access for all users if published or owner/admin" ON public.campaign_content FOR SELECT USING (true);


--
-- Name: campaign Enable update for admin or owner if unpublished; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Enable update for admin or owner if unpublished" ON public.campaign FOR UPDATE USING ((public.is_admin() OR (public.is_owner(user_id) AND (is_published = false)))) WITH CHECK ((public.is_admin() OR (public.is_owner(user_id) AND (is_published = false))));


--
-- Name: campaign_content Enable update for admin or owner if unpublished; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Enable update for admin or owner if unpublished" ON public.campaign_content FOR UPDATE USING ((public.is_admin() OR (public.is_owner(user_id) AND (( SELECT campaign.is_published
   FROM public.campaign
  WHERE (campaign_content.campaign_id = campaign.id)) = false)))) WITH CHECK ((public.is_admin() OR (public.is_owner(user_id) AND (( SELECT campaign.is_published
   FROM public.campaign
  WHERE (campaign_content.campaign_id = campaign.id)) = false))));
...
--
-- Name: campaign_content; Type: ROW SECURITY; Schema: public; Owner: postgres
--
...
--
-- Name: FUNCTION is_admin_or_owner(user_id uuid); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.is_admin_or_owner(user_id uuid) TO anon;
GRANT ALL ON FUNCTION public.is_admin_or_owner(user_id uuid) TO authenticated;
GRANT ALL ON FUNCTION public.is_admin_or_owner(user_id uuid) TO service_role;


--
-- Name: FUNCTION is_owner(user_id uuid); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.is_owner(user_id uuid) TO anon;
GRANT ALL ON FUNCTION public.is_owner(user_id uuid) TO authenticated;
GRANT ALL ON FUNCTION public.is_owner(user_id uuid) TO service_role;
...
--
-- Name: TABLE campaign_content; Type: ACL; Schema: public; Owner: postgres
--
